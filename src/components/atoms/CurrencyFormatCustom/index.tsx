...
export const CurrencyFormatCustom = (props: Props) => {
  ...
  const thousandSeparator = 'DOTS'
  const floatRounding = 2
  const symbol = ''
  const currencyDefault = 'VND'
  const position = 'RIGHT' as 'RIGHT' | 'LEFT'
  const {
    amount = 0,
    color = BLACK,
    className = '',
  ...
  const type = decimalSeparator === 'COMMA' ? 'en-US' : 'de-DE'
